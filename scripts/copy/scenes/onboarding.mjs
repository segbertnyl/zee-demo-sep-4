...
export function rows() {
  ...
  ;[
    ...
    ['onboarding.create-plan.body1', 'create-plan', 'body', "We'll use existing insights about your business, along some questions, to create a plan tailored to your practice and clients."],
    ['onboarding.create-plan.body2', 'create-plan', 'body', 'The next few questions help personalize your experience. It takes about 4 minutes, and you can adjust this information at any time.'],
  ...
  ;[
    ...
    ['onboarding.welcome.body', 'welcome', 'body', "I'm Nyla. Before we start, I want to learn how you work — your book, your rhythm, what you're trying to build. A few quick questions, then I get out of your way."],
  ...
  ;[
    ...
    ['onboarding.profile.subheadline', 'profile', 'body', "We've already gathered some information about your practice. Take a look, confirm what's correct, and update anything that needs attention."],
    ['onboarding.profile.loaded-indicator', 'profile', 'label', 'Profile loaded'],
    ['onboarding.profile.analysis', 'profile', 'body', 'You seem to have a strong foundation in protection products, with opportunities to build toward a more holistic advice practice. The questions ahead focus on your future goals.'],
  ...
  ;[
    ...
    { id: 'product', k: 'Primary product', v: 'Protection', sub: 'Transitioning to holistic', src: 'Source: Salesforce' },
    ...
    out.push(ob('profile', `onboarding.profile.cards.${c.id}.value`, 'stat', c.v, { storage: 'constant', notes: 'Demo value — will be data-driven in production' }))
    if (c.sub) out.push(ob('profile', `onboarding.profile.cards.${c.id}.sub`, 'label', c.sub, { storage: 'constant' }))
    if (c.src) out.push(ob('profile', `onboarding.profile.cards.${c.id}.source`, 'label', c.src, { storage: 'constant' }))
  })

  // Targets step
  ;[
    ['onboarding.targets.body', 'targets', 'body', 'Your manager Elisa F. has set some targets for your development. These factor into how I suggest your goals.'],
  ...
  // Goals intro
  ;[
    ['onboarding.goals-intro.headline', 'goals-intro', 'headline', "Let's define your goals for 2026.", '', 'mode !== "revisit-goals"'],
    ['onboarding.goals-intro.headline-revisit', 'goals-intro', 'headline', "Let's revisit your goals.", '', 'mode === "revisit-goals"'],
    ['onboarding.goals-intro.body', 'goals-intro', 'body', "Set the targets that you want to keep track against. We'll help you figure out the pace and the steps to get there. Don't worry, you can adjust this at any time.", '', 'mode !== "revisit-goals"'],
    ['onboarding.goals-intro.body-revisit', 'goals-intro', 'body', "Walk through each question and update anything that's changed. Your previous answers are already filled in — just adjust what you want and confirm the rest.", '', 'mode === "revisit-goals"'],
  ...
  // Advisor vision step
  ;[
    ['onboarding.advisor-vision.headline', 'advisor-vision', 'headline', 'Anything else you want to share about your vision?'],
    ['onboarding.advisor-vision.sub', 'advisor-vision', 'body', 'In your own words — no right answer.'],
    ['onboarding.advisor-vision.placeholder', 'advisor-vision', 'placeholder', 'e.g. Someone who helps people think about their whole financial picture, not just their policies'],
  ].forEach(([key, step, type, content]) => {
    out.push(ob(step, key, type, content, { storage: 'inline' }))
  })
  ...
  // Direction step
  ;[
    ['onboarding.direction.headline', 'direction', 'headline', 'Where do you want your practice to head in the next 2–3 years?'],
  ...
  ;[
    ...
    ['onboarding.fyc-target.body', 'fyc-target', 'body', "I recommend aiming for $42,000–$51,000 targets based on your past performance and current targets. You can adjust them at any time."],
    ...
    ['onboarding.fyc-target.feedback-conservative', 'fyc-target', 'body', 'A conservative goal that builds momentum.', '', 'val < 35000'],
    ['onboarding.fyc-target.feedback-ambitious', 'fyc-target', 'body', 'This is a comfortably ambitious goal based on your current trajectory.', '', '35000 <= val <= 55000'],
    ['onboarding.fyc-target.feedback-stretch', 'fyc-target', 'body', 'A stretch — possible, but it leaves no slack.', '', 'val > 55000'],
  ...
  ;[
    ...
    ['onboarding.activity-target.sub', 'activity-target', 'body', 'These give your plan leading indicators, beyond outcomes tracking.'],
    ['onboarding.activity-target.guidance', 'activity-target', 'body', "I've suggested a comfortable starting point based on your historic performance and current goals."],
  ...
  // Outside work step
  ;[
    ['onboarding.outside-work.headline', 'outside-work', 'headline', 'What are you looking forward to outside of work this year?'],
    ['onboarding.outside-work.sub', 'outside-work', 'body', "Optional. This is what makes your morning briefing feel like it's on your side, not just your pipeline."],
    ['onboarding.outside-work.placeholder', 'outside-work', 'placeholder', 'Or describe in your own words…'],
  ].forEach(([key, step, type, content]) => {
    out.push(ob(step, key, type, content, { storage: 'inline' }))
  })
  ;['Time with family', 'Travel', 'A wellness and fitness goal', 'A passion project', 'Financial independence', 'More free time'].forEach((t, i) => {
    out.push(ob('outside-work', `onboarding.outside-work.chips[${i}]`, 'tag', t, { storage: 'constant' }))
  })

  // Growth intro
  ;[
    ['onboarding.growth-intro.headline', 'growth-intro', 'headline', "We've talked about your goals; now let's talk about how you want to get there."],
    ['onboarding.growth-intro.body', 'growth-intro', 'body', "We'll synthesize everything — your goals, your book, your calendar — and put the most important things in front of you each day. These answers shape what that feels like."],
  ...
  ;[
    ...
    ['onboarding.progress-areas.sub', 'progress-areas', 'body', "Choose up to 3 — I'll prioritize the work around these."],
  ].forEach(([key, step, type, content]) => {
    out.push(ob(step, key, type, content, { storage: 'inline' }))
  })
  ;[
    ...
    out.push(ob('progress-areas', `onboarding.progress-areas.options.${o.id}.title`, 'label', o.title, { storage: 'constant' }))
    out.push(ob('progress-areas', `onboarding.progress-areas.options.${o.id}.sub`, 'label', o.sub, { storage: 'constant' }))
  })
  ...
  // Clients intro
  ;[
    ['onboarding.clients-intro.headline', 'clients-intro', 'headline', "You've told us where you're headed. Now let's understand how you're spending your time with clients."],
    ['onboarding.clients-intro.body', 'clients-intro', 'body', "This is about how you engage clients, where you want support, and what earns your attention. I'll use those signals to surface the right opportunities, conversations, and next steps at the right time."],
  ...
  ;[
    ...
    ['onboarding.client-signals.sub', 'client-signals', 'body', "Select all that apply. I'll prioritize your alerts in this order."],
  ].forEach(([key, step, type, content]) => {
    out.push(ob(step, key, type, content, { storage: 'inline' }))
  })
  ;[
    ...
    out.push(ob('client-signals', `onboarding.client-signals.options.${o.id}.title`, 'label', o.title, { storage: 'constant' }))
    out.push(ob('client-signals', `onboarding.client-signals.options.${o.id}.sub`, 'label', o.sub, { storage: 'constant' }))
  })

  // Client conversations
  ;[
    ['onboarding.client-conversations.headline', 'client-conversations', 'headline', 'Which client conversations do you want more support with?'],
    ['onboarding.client-conversations.sub', 'client-conversations', 'body', 'Select up to three areas where pre-meeting prep would help the most.'],
  ].forEach(([key, step, type, content]) => {
    out.push(ob(step, key, type, content, { storage: 'inline' }))
  })
  ;[
    ...
    out.push(ob('client-conversations', `onboarding.client-conversations.options.${o.id}.title`, 'label', o.title, { storage: 'constant' }))
  })

  // Stay in front
  ;[
    ['onboarding.stay-in-front.headline', 'stay-in-front', 'headline', 'How do you currently stay connected with clients and prospects between meetings?'],
  ...
  ;[
    ...
    ['onboarding.plan.body', 'plan', 'body', "You set a goal of {fyc}. Let's do the math to figure out a weekly cadence of how many cases and appointments you need to take to get you there.", 'fyc'],
  ...
  // Processing states
  ;['Processing your inputs', 'Personalizing', 'Calibrating', 'Refining your plan', 'Mapping your goals', 'Almost there'].forEach((t, i) => {
