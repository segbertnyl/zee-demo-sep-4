...
const allRows = [
  ...
  ...briefingRows(),
  ...discoveryRows(),
].filter(Boolean).map((row, i) => ({ ...row, ID: i + 1 }))
...
// Summary
const byScene = {}
allRows.forEach(r => { byScene[r.Scene] = (byScene[r.Scene] || 0) + 1 })
...
if (existsSync(SA_PATH)) {
  ...
  const sheetId = meta.data.sheets.find(s => s.properties.title === 'Copy')?.properties.sheetId
...
console.log('\nBatch breakdown:')
Object.entries(byScene).sort((a,b)=>b[1]-a[1]).forEach(([s,n]) => console.log(`  ${s}: ${n} rows`))
