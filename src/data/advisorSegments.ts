/* Advisor segments — ported from Zee's NYL360_FullOnboarding prototype.
 *
 * The whole point of segment-aware onboarding: an experienced top-performer
 * gets framed at peer level; a holistic-accelerating advisor gets sequenced
 * direction; a brand-new advisor gets practical-rhythm framing. Same flow,
 * different copy.
 *
 * Each entry carries:
 *   - identity data (name, tenure, FYC, council standing, etc.)
 *   - segment-conditional question content per archetype (Coach growth
 *     areas, Analyst priorities, Strategist directions, Concierge friction)
 *   - plan-math seed (close rate by tenure, average case size, etc.) */

export type SegmentCode = 'cs-leading' | 'hl-accelerating' | 'cs-building'

export type SegmentOption = { id: string; label: string; sub?: string }

export type AdvisorSegment = {
  code: SegmentCode
  ...
  intro: {
    coachIdentityPrompt: string
    coachIdentityPlaceholder: string
    coachConsentLabel: string
    brandVoicePlaceholder: string
    chiefBigGoalPlaceholder: string
  }

  growthAreas: SegmentOption[]
  analystMorning: SegmentOption[]
  analystSignals: SegmentOption[]
  strategistDirections: SegmentOption[]
  conciergeSlows: SegmentOption[]
  conciergeTimeSuggestions: string[]
  /* Concierge "if you could eliminate one thing" question prompt — different
   * framing per segment (Zee's eliminatePrompt). */
  conciergeEliminatePrompt: string
  /* Brand Advocate — which channels matter to this segment. */
  brandChannels: SegmentOption[]
  /* Brand Advocate — life-event instinct options. cs-leading has its own set;
   * accelerating + building share one. */
  brandLifeEventInstincts: SegmentOption[]
  /* Nyla — how the OS should show up each day. Different count + copy
   * per segment (4 / 5 / 4 in Zee's source). */
  chiefOsStyleOptions: SegmentOption[]
}

/* ----------------------------------------------------------------------------
 * Marcus T. — Core Specialist · Leading. 16-year veteran, Executive Council.
 * Frame: peer level, not coaching basics.
 * -------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------------
 * Priya M. — Holistic Leaning · Accelerating. 14-year, in transition.
 * Frame: sequenced guidance, clear path forward.
 * -------------------------------------------------------------------------- */
...
/* ----------------------------------------------------------------------------
 * Jordan R. — Core Specialist · Building. 2-year, early career.
 * Frame: practical rhythm, income clarity, confidence building.
 * -------------------------------------------------------------------------- */
...
export const SEGMENTS: Record<SegmentCode, AdvisorSegment> = {
  'cs-leading': csLeading,
  'hl-accelerating': hlAccelerating,
  'cs-building': csBuilding,
}

export const DEFAULT_SEGMENT: SegmentCode = 'hl-accelerating'
