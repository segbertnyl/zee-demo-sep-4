...
export function PlanDrawer({ open, goals, onFinish }: { open: boolean; goals: Goals; onFinish: () => void }) {
  ...
  return <AnimatePresence>{open && <PlanDrawerInner goals={goals} onFinish={onFinish} />}</AnimatePresence>
}
            ...
            {PLAN_STEPS.slice(0, active + 1).map((id, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={['py-10', isActive ? '' : 'pointer-events-none select-none'].join(' ')}
                >
...
function renderPlanStep(id: PlanStepId, p: PlanStepProps): ReactNode {
  switch (id) {
    case 'qualifications':
      return <Qualifications advance={p.advance} />
    case 'pace':
      return <Pace advance={p.advance} />
    case 'adjustments':
      return <Adjustments advance={p.advance} />
    case 'milestones':
      return <Milestones advance={p.advance} />
    case 'preferences':
      return <Preferences onFinish={p.onFinish} />
  }
}
    ...
    <div className="flex flex-col gap-7">
      <div className="flex items-start gap-3">
        ...
        <h2
          ...
          style={{ fontWeight: 400, fontFamily: 'var(--font-serif)', textWrap: 'balance' }}
        >
          You're on track to meet your FYC target, but you had a slower start this year and will need to increase your
          pace to hit the Executive Council goal.
        </h2>
      </div>
      ...
      <p className="max-w-[64ch] text-[13.5px] leading-[1.55] text-white/80">
        This is what your year projected pace is looking like and I'm seeing several gaps to fill. Would you like to see
        how we can adjust your current pace?
      </p>
...
function Adjustments({ advance }: { advance: () => void }) {
  return (
    <div className="flex flex-col gap-7">
      ...
      <p className="max-w-[64ch] text-[13.5px] leading-[1.55] text-white/80">
        I've looked through your book and found a few opportunities from existing clients for you to get started with.
        If you close a large case, your monthly target relaxes. If you fall behind, your OS identifies the fastest path
        to recover.
      </p>
    ...
    <div className="flex flex-col gap-7">
      <div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3">
          ...
          <StatCard
            eyebrow="Cases to close"
            value="1–2 /mo"
            sub="based on your avg case size"
            tag="On track"
            tone="good"
          />
          <StatCard
            eyebrow="Client appointments"
            value="7"
            sub="to generate your close rate"
            tag="On track"
            tone="good"
          />
          <StatCard
            eyebrow="Prospect contacts"
            value="4 /week"
            sub="to fill your appointment pipeline"
            tag="On track"
            tone="good"
          />
          <StatCard
            ...
            value="5"
            sub="to protect and deepen the book"
            tag="On track"
            tone="good"
          />
      ...
      <div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <StatCard
            eyebrow="Cases needed"
            value="+12 cases"
            sub="6 months · 2 cases per mo"
            tag="Stretch"
            tone="warn"
          />
        </div>
      ...
      <div className="flex flex-col gap-4">
        <p className="max-w-[64ch] text-[13.5px] leading-[1.55] text-white/80">
          I recommend you focus on new premium protection cases, with each one adding an average of $3,200 in-force
          premium. You'll need to reach 12 more protection cases across 6 months.
        </p>
        <p className="max-w-[64ch] text-[13.5px] leading-[1.55] text-white/70">
          If this looks good to you, I'd like to review your personal ways of working before I build out your plan.
        </p>
      </div>
    ...
    <div className="flex flex-col gap-7">
      ...
      <div>
        ...
        <p className="mt-3 max-w-[68ch] text-[14px] leading-[1.6] text-white/85">
          You prioritize relationship depth over pipeline velocity. You're in it for the long conversation, not the next
          close. Protection is your foundation. Human connection is your method. Holistic planning is where you're
          headed.
        </p>
      </div>
...
function ContributionBar({
  ...
  segments: { label: string; value: string; weight: number; color: string }[]
}) {
    ...
    <div className="rounded-lg border border-white/12 bg-white/[0.05] p-4">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="Projected pace chart">
        ...
        <line
          ...
          strokeDasharray="2 4"
        />
        <text x={W - padR} y={councilGoalY - 6} fill="#cdbff0" fontSize="10" textAnchor="end">
          $100K
        </text>
        <line
          x1={padL}
          ...
          strokeDasharray="2 4"
        />
        <text x={W - padR} y={fycGoalY - 6} fill="rgba(255,255,255,0.7)" fontSize="10" textAnchor="end">
          $50K
        </text>
        ...
        <line
          ...
          strokeWidth={1}
        />
        <text x={x(todayT)} y={padT - 12} fill="rgba(255,255,255,0.85)" fontSize="10" textAnchor="middle">
          Today, Dec 16
        </text>
      ...
      {dashed || dotted ? (
        <svg width="16" height="6" aria-hidden="true">
          <line
            ...
            stroke={swatch}
            strokeWidth={2}
            strokeDasharray={dotted ? '1.5 2.5' : '4 3'}
          />
        </svg>
      ) : (
