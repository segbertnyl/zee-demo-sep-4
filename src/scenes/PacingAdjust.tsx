...
function PacingSlider({ config, value, onChange }: PacingSliderProps) {
  ...
  const lastYearIdx = config.lastYear != null ? Math.round((config.lastYear - config.min) / config.step) : null
            ...
            >
              ...
              <div
                style={{
                  ...
                  background: isThumb ? thumbColor : isRec ? 'var(--nyl-purple-500)' : 'var(--nyl-gray-200, #d0ccc8)',
    ...
    >
      ...
      {open && !editing && (
        ...
        >
          ...
          >
            <span>
              <Bold>${fmtK(FYC_GOAL)} FYC</Bold> ÷ <Bold>~${fmtK(AVG_COMMISSION)} /policy</Bold> ={' '}
              <Bold>~{fmtK(REC_CLOSES_PER_YEAR)} closes /yr</Bold>
            </span>
            <span>
              <Bold>~{fmtK(REC_CLOSES_PER_YEAR)} closes /yr</Bold> ÷{' '}
              <Bold>{Math.round(CLOSE_RATE * 100)}% close rate</Bold> ={' '}
              <Bold>~{fmtK(REC_APPTS_PER_YEAR)} appts /yr</Bold>
            </span>
            <span>
              <Bold>~{fmtK(REC_APPTS_PER_YEAR)} appts /yr</Bold> ÷ <Bold>52 wks</Bold> ={' '}
              <Bold>~{REC_APPTS_PER_WEEK} appts /wk</Bold>
            </span>
          </div>
          <button
            type="button"
...
export function PacingAdjust({ onSkip, onConfirm }: PacingAdjustProps) {
  ...
  const numberColor = status === 'on-track' ? 'var(--text-heading)' : 'var(--nyl-orange-500)'
...
function buildGuidanceBody(outSliders: SliderConfig[], values: Record<string, number>): ReactNode {
  ...
  if (prospects) {
    ...
    if (needed > 0) {
      return (
        <>
          Looking at the current size of your book, you would need to add <strong>~{needed} more prospects /wk</strong>{' '}
    ...
    return (
      <>
        You're targeting more prospects than recommended. Reducing to <strong>~{prospects.recommendedMax}/wk</strong>{' '}
  ...
  if (referrals) {
    ...
    if (v < referrals.recommendedMin) {
      return (
        <>
          Increasing referral asks to at least <strong>{referrals.recommendedMin}/mo</strong> would significantly
          improve your qualified appointment volume.
        </>
      )
    }
    return (
      <>
        That's a strong referral target. Stay within <strong>{referrals.recommendedMax}/mo</strong> to keep your
        follow-up quality high.
      </>
    )
  }
  ...
  if (networking) {
    return (
      <>
        Targeting <strong>{values[networking.id]} networking events/mo</strong> puts you outside the recommended range
        of {networking.recommendedMin}–{networking.recommendedMax}. Adjusting within range keeps your pipeline balanced.
      </>
    )
  }
