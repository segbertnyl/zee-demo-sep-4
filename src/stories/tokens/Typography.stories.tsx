  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {SCALE.map(({ token, lineToken, label, size, line }) => (
        <div key={token} style={{ display: 'flex', alignItems: 'baseline', gap: 24, borderBottom: '1px solid var(--border-subtle)', paddingBottom: 12 }}>
          <div style={{ width: 80, flexShrink: 0, fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-body-muted)' }}>
            <span style={{ display: 'block' }}>{label}</span>
            <span style={{ display: 'block' }}>{size}/{line}</span>
      ...
      {WEIGHTS.map(({ name, value, token }) => (
        <div key={token} style={{ display: 'flex', alignItems: 'baseline', gap: 24, borderBottom: '1px solid var(--border-subtle)', paddingBottom: 12 }}>
          <div style={{ width: 120, flexShrink: 0 }}>
            <span style={{ fontFamily: 'var(--font-sans)', fontSize: 11, color: 'var(--text-body-muted)', display: 'block' }}>{name}</span>
            <code style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-body-faint)' }}>{token} ({value})</code>
          </div>
          <p style={{
            fontFamily: 'var(--font-sans)',
            fontSize: 24,
...
export const UtilityClasses: StoryObj = {
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        <code style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-body-muted)', display: 'block', marginBottom: 8 }}>.headline</code>
        <p className="headline" style={{ fontSize: 32, margin: 0 }}>Serif headline, editorial restraint</p>
      </div>
      <div>
        <code style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-body-muted)', display: 'block', marginBottom: 8 }}>.eyebrow</code>
        <p className="eyebrow" style={{ margin: 0 }}>Eyebrow label — section divider</p>
      </div>
      <div>
        <code style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-body-muted)', display: 'block', marginBottom: 8 }}>.accent</code>
        <p className="accent" style={{ fontSize: 16, fontFamily: 'var(--font-sans)', margin: 0 }}>Accent text in brand blue</p>
      </div>
    </div>
  ),
}
