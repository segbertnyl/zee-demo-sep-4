...
function PlaygroundPanel() {
    ...
    const persisted = (() => {
      try { return JSON.parse(sessionStorage.getItem('nyla-playground') ?? 'null') } catch { return null }
    })() as { tier?: NylaSizeTier; view?: string; bgIdx?: number } | null
    const startTier: NylaSizeTier = persisted?.tier != null && (NYLA_SIZE_TIERS as readonly number[]).includes(persisted.tier) ? persisted.tier : 160
    ...
    const startView = persisted?.view && listViewsForTier(startTier).includes(persisted.view) ? persisted.view : DEFAULT_VIEW
    ...
    const onNewView = () => {
      ...
      saveOrbConfigForTier(tier, name, { dotScale, glowScale, minOpacity, rotationSpeed, purpleMix, haloStrength, points: auto ? null : points })
    ...
    const onPoints = (v: number) => { setPoints(v); setAuto(false) }
    const onAuto = (checked: boolean) => { setAuto(checked); if (checked) setPoints(recommendedPoints(size)) }

    const handleSave = () => {
      saveOrbConfigForTier(tier, view, { dotScale, glowScale, minOpacity, rotationSpeed, purpleMix, haloStrength, points: auto ? null : points })
      setSaveState('saved')
      setTimeout(() => setSaveState('idle'), 2000)
    }
    ...
    const sliderRow = (label: string, value: number, min: number, max: number, step: number, display: string, onChange: (v: number) => void) => (
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
          <span style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.12em', color: labelColor }}>{label}</span>
          <span style={{ fontSize: 13, fontVariantNumeric: 'tabular-nums', color: textMuted, background: inputBg, padding: '2px 8px', borderRadius: 4, fontFamily: 'var(--font-mono, monospace)' }}>{display}</span>
        </div>
        <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(+e.target.value)} style={{ width: '100%', accentColor: NYL.blue, cursor: 'pointer' }} />
      </div>
    )

    return (
      <div style={{ minHeight: '100vh', background: bg.css, color: textColor, fontFamily: 'var(--font-sans)', display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', alignItems: 'stretch', transition: 'background 0.35s ease' }}>

        {/* Stage */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48 }}>
          <div style={{ width: 'min(46vw, 560px)', aspectRatio: '1 / 1', borderRadius: 24, background: isDark ? 'rgba(255,255,255,0.06)' : `${NYL.navy}08`, display: 'grid', placeItems: 'center', boxShadow: isDark ? '0 24px 70px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.08)' : '0 24px 70px rgba(0,10,98,0.16)', overflow: 'hidden', border: `1px solid ${isDark ? 'rgba(255,255,255,0.1)' : `${NYL.navy}12`}` }}>
            <PointSphere size={size} count={points} animate dotScale={dotScale} glowScale={glowScale} minOpacity={minOpacity} rotationSpeed={rotationSpeed} purpleMix={purpleMix} haloStrength={haloStrength} />
          </div>
        </div>

        {/* Controls */}
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 24, padding: '48px 64px 48px 24px', maxWidth: 560, borderLeft: `1px solid ${borderColor}` }}>

          <div>
            <h1 style={{ fontFamily: 'var(--font-serif)', fontWeight: 400, fontSize: 40, lineHeight: 1.05, margin: 0, color: textColor }}>The Collective</h1>
            <p style={{ margin: '12px 0 0', fontSize: 15, lineHeight: 1.55, color: textMuted }}>Dial in the look. Hit Save to apply across the prototype immediately.</p>
          </div>

          {/* Divider — Size tier */}
          <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.18em', color: textMuted, borderBottom: `1px solid ${borderColor}`, paddingBottom: 8 }}>Size tier — each saves its own view</div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: textMuted, marginRight: 4 }}>View</span>
            {views.map((v) => (
              <button
                key={v}
                onClick={() => onView(v)}
            ...
            <button
              ...
              style={{ cursor: 'pointer', padding: '6px 12px', borderRadius: 999, fontSize: 12, fontWeight: 600, background: 'transparent', color: labelColor, border: `1px dashed ${labelColor}` }}
            >
              ＋ New view
            </button>
          </div>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, cursor: 'pointer', color: textColor }}>
            <input type="checkbox" checked={auto} onChange={(e) => onAuto(e.target.checked)} style={{ width: 16, height: 16, accentColor: NYL.blue, flexShrink: 0 }} />
            Match density to size
            <span style={{ fontSize: 12, color: textMuted }}>({recommendedPoints(size)} pts)</span>
          </label>

          {/* Divider — Style */}
          <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.18em', color: textMuted, borderBottom: `1px solid ${borderColor}`, paddingBottom: 8 }}>Style</div>

          {sliderRow('Dot size', dotScale, 0.01, 0.20, 0.005, dotScale.toFixed(3), setDotScale)}
          {sliderRow('Glow', glowScale, 0.005, 0.08, 0.002, glowScale.toFixed(3), setGlowScale)}
          {sliderRow('Min opacity', minOpacity, 0.05, 0.90, 0.05, minOpacity.toFixed(2), setMinOpacity)}
          {sliderRow('Rotation', rotationSpeed, 0, 2, 0.05, rotationSpeed === 0 ? 'still' : `${rotationSpeed.toFixed(2)} rad/s`, setRotationSpeed)}
          {sliderRow('Purple', purpleMix, 0, 1, 0.01, `${Math.round(purpleMix * 100)} / 100`, setPurpleMix)}
          {sliderRow('Halo', haloStrength, 0, 2, 0.05, haloStrength === 0 ? 'off' : `${haloStrength.toFixed(2)}×`, setHaloStrength)}

          {/* Background */}
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <span style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: textMuted, marginRight: 4 }}>Background</span>
            {BGS.map((b, i) => (
              <button key={b.id} onClick={() => setBgIdx(i)} title={b.label} style={{ cursor: 'pointer', width: 30, height: 30, borderRadius: 8, background: b.css, border: bg.id === b.id ? `2px solid ${NYL.blue}` : `1px solid rgba(128,128,128,0.35)`, outline: bg.id === b.id ? `2px solid ${NYL.blue}40` : 'none', transition: 'border 0.15s ease' }} />
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', paddingTop: 4 }}>
            <button
              onClick={handleSave}
              style={{ flex: 1, padding: '10px 0', borderRadius: 8, background: saveState === 'saved' ? '#16a34a' : NYL.blue, color: '#fff', border: 'none', fontSize: 13, fontWeight: 700, letterSpacing: '0.06em', cursor: 'pointer', transition: 'background 0.2s' }}
            >
              {saveState === 'saved' ? `✓ Saved ${tier}px · ${view} — prototype updates live` : `Save ${tier}px · ${view}`}
            </button>
            <button
              onClick={handleReset}
              style={{ padding: '10px 16px', borderRadius: 8, background: 'transparent', color: textMuted, border: `1px solid ${borderColor}`, fontSize: 12, cursor: 'pointer' }}
            >
          ...
          {/* Code snippet */}
          <pre style={{ margin: 0, padding: '12px 14px', borderRadius: 8, background: inputBg, border: `1px solid ${borderColor}`, fontSize: 11, lineHeight: 1.7, color: textMuted, fontFamily: 'var(--font-mono, monospace)', overflow: 'auto' }}>
