...
function Fixture({ label, dark = false, children }: { label: string; dark?: boolean; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <div
        className={`rounded-2xl border p-6 ${dark ? 'border-white/10 bg-[var(--nyl-blue-800)]' : 'border-neutral-100 bg-[var(--bg-surface)]'}`}
      >
        {children}
      </div>
    </div>
  ...
  render: () => (
    <div className="flex flex-col gap-8">
      ...
      <Fixture label="Single primary">
        <SectionActions actions={[{ label: 'Continue', variant: 'primary' }]} />
      </Fixture>
