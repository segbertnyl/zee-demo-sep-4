...
// ── Domain icons (match src/scenes/PlanSummary.tsx) ─────────────────────────────
...
// ── Card content (matches src/scenes/PlanSummary.tsx) ───────────────────────────
  ...
  items: [
    {
      ...
      chips: [
        { icon: 'growth' as const, label: 'Go holistic' },
        { icon: 'council' as const, label: 'Reach Executive Council' },
      ],
    },
    {
      ...
      chips: [
        { icon: 'succession' as const, label: 'Succession' },
        { icon: 'fyc' as const, label: 'Boost FYC' },
      ],
    },
  ],
  nylaItems: [
  ...
  items: [
    {
      ...
      chips: [
        { icon: 'fyc' as const, label: 'Boost FYC' },
        { icon: 'council' as const, label: 'Reach Executive Council' },
        { icon: 'network' as const, label: 'Broaden network' },
      ],
    },
    {
      ...
      chips: [{ icon: 'client' as const, label: 'Prioritize current clients' }],
    },
  ],
  nylaItems: [
  ...
  items: [
    {
      ...
      chips: [{ icon: 'succession' as const, label: 'Succession' }],
    },
  ],
  nylaItems: [
...
} satisfies Meta<typeof PlanActions>

type Story = StoryObj<typeof PlanActions>

export const Playground: Story = {
  name: 'Playground — My practice',
  args: { icon: <PracticeIcon />, ...PRACTICE },
}

export const MyProduction: Story = {
  name: 'My production',
  args: { icon: <ProductionIcon />, ...PRODUCTION },
}

export const MyLifeBalance: Story = {
  name: 'My life balance',
  args: { icon: <LifeBalanceIcon />, ...LIFE_BALANCE },
}

// The full "How we'll get there" section as it appears on the Plan Summary
// (plan reveal): all three domain cards stacked on the purple bokeh background.
export const HowWeGetThere: Story = {
  name: "How we'll get there — 3 cards",
  ...
  render: () => (
    <div style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '80px 340px 96px 80px' }}>
        <p
          style={{
            margin: 0,
        ...
        {/* Width = content + 243px pill area so the flex math gives cards the full
            content width; hover pills overflow to the right (matches PlanSummary). */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: 'calc(100% + 243px)' }}>
          <PlanActions icon={<PracticeIcon />} {...PRACTICE} />
          <PlanActions icon={<ProductionIcon />} {...PRODUCTION} />
          <PlanActions icon={<LifeBalanceIcon />} {...LIFE_BALANCE} />
        </div>
      </div>
    </div>
  ),
