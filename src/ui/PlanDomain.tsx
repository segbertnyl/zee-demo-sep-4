        ...
        {committed.map((item) => (
          <motion.div
            ...
            onKeyDown={
              editing
                ? (e) => {
                    if (e.key === 'Enter' || e.key === ' ') setCommitted(item.id, false)
                  }
                : undefined
            }
            style={{ ...rowBase, cursor: editing ? 'pointer' : 'default' }}
          >
            <span style={{ width: 32, display: 'flex', justifyContent: 'center', paddingTop: 2, flexShrink: 0 }}>
              {editing ? <XLight /> : <CheckLight />}
            </span>
