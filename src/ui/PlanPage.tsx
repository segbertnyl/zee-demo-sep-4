...
import { PlanDomain } from '@/ui/PlanDomain'
import { PaceChart } from '@/ui/PaceChart'
import { PageSubnav } from '@/ui/PageSubnav'
import { OverviewWins, PlanSummaryLists } from '@/ui/PlanOverviewExtras'

/* Plan page — Exploration pt-II (Figma 1327-34713 Overview / 1327-35971 Practice).
 *
 * Rendered inside the Briefing v6 scene when the rail's "Plan" destination is
 * active (replacing the not-built placeholder). The scene supplies the purple
 * bokeh background and the left rail; this component owns the scrollable content
 * and the floating bottom subnav that pages between the four sections.
 *
 * Content baseline is the Discovery Plan-summary (src/scenes/PlanSummary.tsx) —
 * the goal statement, pacing chart, and per-domain actions all match. Overview +
 * Practice are built; Production + Life Balance are stubbed pending design review. */

const SECTIONS = ['Overview', 'Practice', 'Production', 'Life Balance'] as const
type Section = (typeof SECTIONS)[number]

// ── Pacing chart data ───────────────────────────────────────────────────────────
// x-axis: Mar (0) · July (286) · Nov (572) · Jun (858). Today is Dec 12, so the
// marker sits just past the Nov gridline (~630); actual runs Mar→Dec, projections
// fan out Dec→Jun.
const TODAY_X = 630
const CHART_ACTUAL = [
  { x: 0, value: 2 },
...
const CHART_CURRENT_PACE = [
  { x: TODAY_X, value: 22 },
  { x: 710, value: 25 },
  { x: 784, value: 29 },
  { x: 858, value: 32.4 },
]
const CHART_PLAN_PROJ = [
  { x: TODAY_X, value: 22 },
  { x: 710, value: 30 },
  { x: 784, value: 39 },
  { x: 858, value: 47 },
...
// ── Domain icons + heading (Figma 1327-36024 / 1359-51363 / 1359-53020) ────────────
// Section heading = a lavender icon tile + the domain name in Alverata (24/32).
...
function ProductionIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs><clipPath id="pp-prod-clip"><rect width="28" height="28" fill="white"/></clipPath></defs>
    </svg>
  )
}
...
const OVERVIEW_HEADLINE =
  'This is the year you reach Executive Council, deliver holistic advice, and build the kind of practice that changes lives — without sacrificing your own.'
...
/* Per-word type-in — mirrors the Briefing headline's blur-to-sharp reveal. Plays
 * on mount (so it runs on the initial page load); reduced-motion gets it instantly. */
function OverviewHeadline({ animate }: { animate: boolean }) {
  if (!animate || prefersReducedMotion()) return <p style={HEADLINE_STYLE}>{OVERVIEW_HEADLINE}</p>

  const words = OVERVIEW_HEADLINE.split(' ')
  return (
    <p style={HEADLINE_STYLE} aria-label={OVERVIEW_HEADLINE}>
      {words.map((w, i) => (
        <Fragment key={i}>
          <motion.span
            aria-hidden="true"
            ...
            transition={{ duration: 0.9, delay: (i * 63) / 1000, ease: EASE.settle as [number, number, number, number] }}
...
// Initial-load pause (matches the Briefing's 1500ms chrome-only hold) and the wins
// reveal offset (~headline type-in 2.4s + 500ms).
const OVERVIEW_LOAD_PAUSE_MS = 1500
const WINS_DELAY_S = 2.8

/* Play the staged entrance once — on the initial page load. Module-level so it
 * survives section switches; the flag is set only AFTER the pause fires, so
 * StrictMode's discarded first mount (whose timer is cleared) can't skip it. */
let planOverviewLoaded = false

function OverviewSection() {
  const [animateIn] = useState(() => !prefersReducedMotion() && !planOverviewLoaded)
  const [revealed, setRevealed] = useState(!animateIn)
  useEffect(() => {
    if (!animateIn) { planOverviewLoaded = true; return }
    const t = window.setTimeout(() => { planOverviewLoaded = true; setRevealed(true) }, OVERVIEW_LOAD_PAUSE_MS)
    return () => window.clearTimeout(t)
  }, [animateIn])

  // During the initial pause, show chrome only (rail + "Plan" band are already up).
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      transition={{ duration: DURATION.standard, ease: EASE.settle as [number, number, number, number] }}
    >
      {/* Hero — left 2 cols: eyebrow + headline + updated date; right col: the wins. */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 40, alignItems: 'flex-start' }}>
        <div style={{ gridColumn: '1 / 3' }}>
          <SectionLabel>What you&apos;re working towards</SectionLabel>
          <div style={{ marginTop: 32 }}>
            <OverviewHeadline animate={animateIn} />
          </div>
          {/* Updated date — below the headline, left-aligned. Prototype "today" is
              Dec 12. #dcd9d5 = sand on dark; no token. */}
          <p style={{
            margin: '16px 0 0',
            fontFamily: 'var(--font-sans)',
            fontSize: 14,
        ...
        {/* Where you're winning — first 2 wins. Blur-fades in 500ms AFTER the headline
            finishes typing (initial load only); instant on tab-return / reduced motion. */}
        <motion.div
          style={{ gridColumn: '3 / 4' }}
          initial={animateIn ? { opacity: 0, filter: 'blur(8px)' } : false}
          animate={{ opacity: 1, filter: 'blur(0px)' }}
          transition={{ duration: 0.8, delay: animateIn ? WINS_DELAY_S : 0, ease: EASE.settle as [number, number, number, number] }}
        >
          <SectionLabel>Where you&apos;re winning</SectionLabel>
          <div style={{ marginTop: 32 }}>
            <OverviewWins limit={2} />
          </div>
        </motion.div>
      </div>

      {/* Pacing chart — rgba(0,10,98,0.2) dark panel per Figma; no NYL token */}
      <div style={{
        marginTop: 40,
        ...
        <PaceChart
          ...
          neededPacePoints={CHART_PLAN_PROJ}
          todayX={TODAY_X}
          todayLabel="Today, Dec 12"
          actualValueAtToday={22}
          fycGoal={47}
          legend={CHART_LEGEND}
          height={150}
          labelFontSize={10}
          ...
          yMaxPx={32}
          xAxisLabels={[
            { x: 0, label: 'Mar', anchor: 'start' },
            { x: 286, label: 'July' },
            { x: 572, label: 'Nov' },
            { x: 858, label: 'Jun', anchor: 'end' },
          ]}
          actualColor="white"
          currentPaceColor="#ff9624"
          neededPaceColor="#d3a1f7"
      ...
      {/* Collapsed Goals / Practice summary (mirrors Discovery "what I heard") */}
...
function PracticeSection() {
  return (
    <div>
      <SectionHeading icon={<PracticeIcon />}>My practice</SectionHeading>
      <div style={{ marginTop: 32 }}>
        <PlanDomain
          items={[
            {
              ...
              insight: {
                analysis: [
                  'You referred out 9 planning cases this year — up from 6 last year as more clients ask for full planning.',
                  '7 of those 9 needed just a Long-Term Care or Series 65 license — both within reach this year.',
                  'Holistic advisors keep 4 of 5 of these in-house; it’s your single fastest lever to Executive Council.',
                ],
                badge: '$14K referred out — your biggest leak',
              },
            },
            {
              ...
              insight: {
                analysis: [
                  'Two colleagues in your GO are ~12 months from transition — a combined 180 households.',
                  'You’re the front-runner on David Okafor’s book; landing it skips ~2 years of prospecting.',
                ],
                badge: '180 households in play nearby',
              },
            },
          ]}
          nylaItems={[
            { id: 'renewals', label: 'Keep you on track for renewals', static: true },
            { id: 'succession', label: 'Notify you of succession opportunities' },
            { id: 'licensing', label: 'Prep for licensing' },
          ]}
        />
      </div>
    </div>
...
function ProductionSection() {
  return (
    <div>
      <SectionHeading icon={<ProductionIcon />}>My production</SectionHeading>
      <div style={{ marginTop: 32 }}>
        <PlanDomain
          items={[
            {
              title: 'Improve your appointment volume',
              description: 'More conversations is your clearest path to higher FYC.',
              chips: [
                { icon: 'fyc', label: 'Boost FYC' },
                { icon: 'council', label: 'Reach Executive Council' },
                { icon: 'network', label: 'Broaden network' },
              ],
              insight: {
                analysis: [
                  'You’re averaging 5 appointments a week, up from 3 last quarter — a 67% jump.',
                  'Your close rate is 1 in 3, ahead of your GO’s 1-in-4 average.',
                  'Two more a week is the exact pace that locks your $47K goal — the math takes over from there.',
                ],
                badge: '↑ 67% this quarter — 3 to 5 a week',
              },
            },
            {
              ...
              insight: {
                analysis: [
                  '9 of your top-20 clients are overdue for a review.',
                  'Reviews convert ~3× better than cold outreach — about $1,800 in FYC each.',
                ],
                badge: '4 reviews cleared — your best month yet',
              },
            },
          ]}
          nylaItems={[
            { id: 'reviews', label: 'Flag overdue reviews', static: true },
            { id: 'notes', label: 'Synthesize meeting notes', static: true },
            { id: 'holistic', label: 'Prep for holistic conversations', static: true },
            { id: 'outreach', label: 'Draft outreach for priority clients', static: true },
            { id: 'networking', label: 'Uncover networking events that match my interests' },
          ]}
        />
      </div>
    </div>
...
function LifeBalanceSection() {
  return (
    <div>
      <SectionHeading icon={<LifeBalanceIcon />}>My life balance</SectionHeading>
      <div style={{ marginTop: 32 }}>
        <PlanDomain
          items={[
            {
              ...
              insight: {
                analysis: [
                  'You’ve taken 3 days off in the last 90 — down from 8 over the same stretch last year.',
                ],
                badge: 'Council-level advisors take 2.6× more time off',
              },
            },
          ]}
          nylaItems={[
            { id: 'time', label: 'Protect time on your calendar', static: true },
            { id: 'balance', label: 'Track work-life balance' },
          ]}
        />
      </div>
    </div>
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <div style={{ padding: '0 max(40px, 4%) 160px' }}>
          {/* Top band — 120px tall so "Plan" centers on the rail's NYL logo
              (center 60px), matching the Briefing page. */}
          ...
          {/* Offset so page content starts at the same Y as the Briefing headline (168px). */}
          <AnimatePresence mode="wait">
            <motion.div key={section} {...SECTION_MOTION} style={{ marginTop: 32 }}>
              {section === 'Overview' && <OverviewSection />}
              {section === 'Practice' && <PracticeSection />}
              {section === 'Production' && <ProductionSection />}
              {section === 'Life Balance' && <LifeBalanceSection />}
            </motion.div>
          </AnimatePresence>
        </div>
      </div>

      {/* Floating bottom subnav — vertically centered on the rail avatar (center 60px
          from the viewport bottom), matching the Briefing subnav's Y. */}
      <div className="pointer-events-none absolute left-0 right-0 z-20 flex justify-center" style={{ bottom: 60, transform: 'translateY(50%)' }}>
        <PageSubnav
          active={section}
          options={[...SECTIONS]}
